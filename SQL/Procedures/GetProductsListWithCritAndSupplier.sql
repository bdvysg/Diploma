CREATE OR ALTER PROCEDURE GetProductsListWithCritAndSupplier  -- Процедура выводит список продуктов для формы Product
...
-- Create a temporary table to store the results
 ...
 SELECT P.Pr_Id,
  ...
  WHERE  (CONCAT(b.Br_Title, c.Catg_Title, p.Pr_Title) LIKE '%' + @SearchText + '%') AND
          p.Pr_Supplier = @SupId
  ORDER BY p.Pr_Id
...
-- exec GetProductsListWithCritAndSupplier '', 0, 10, 16